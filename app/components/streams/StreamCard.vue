...
<template>
  ...
  >
    ...
    >
      <img
        ...
        :class="
          stream.isLive
            ...
            : 'brightness-50 grayscale group-hover:grayscale-0 group-hover:brightness-75'
        "
      />
      ...
      <div
        v-if="stream.isLive"
        class="absolute top-3 left-3 bg-red-600 text-white font-black text-xs uppercase px-2 py-1 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-1.5 z-10"
      >
      ...
      <div
        v-else
        class="absolute top-3 left-3 bg-stone-700 text-stone-200 font-black text-xs uppercase px-2 py-1 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-1.5 z-10"
      >
        Offline
      </div>
      ...
      <div
        v-if="stream.isLive"
        class="absolute bottom-3 left-3 bg-black/80 backdrop-blur-sm text-white font-bold text-xs px-2 py-1 border border-black flex items-center gap-1.5 z-10"
      >
        <UIcon
    ...
    <!-- Info Area -->
    <div
      class="p-5 flex gap-4 bg-stone-900 group-hover:bg-stone-800 transition-colors duration-300"
    >
      <div class="shrink-0">
        <UAvatar
      ...
      <div class="flex-1 min-w-0">
        <h3
          class="font-heading font-black text-xl text-white truncate drop-shadow-[1px_1px_0_rgba(0,0,0,1)]"
        >
          {{ stream.streamerName }}
        </h3>
        <p
          class="text-sm font-bold truncate mt-0.5"
          :class="stream.isLive ? 'text-stone-300' : 'text-stone-500'"
          :title="stream.title"
        >
          {{
            stream.title && stream.title !== "Offline"
              ? stream.title
              : "Offline"
          }}
        </p>
        <p
          v-if="stream.game"
          class="text-xs font-bold mt-2 uppercase tracking-wide"
          :class="stream.isLive ? 'text-purple-400' : 'text-stone-400'"
        >
          {{
            stream.isLive ? stream.game : "Last seen streaming " + stream.game
          }}
        </p>
      </div>
    </div>

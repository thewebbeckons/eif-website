...
const handleCatClick = (e: MouseEvent) => {
  ...
  setTimeout(() => {
    explodingCats.value = explodingCats.value.filter(
...
<template>
  <section id="unique" class="space-y-16 pointer-events-none mt-24 md:mt-40">
    <div class="text-center space-y-4 pointer-events-auto">
      <h2
        class="text-6xl md:text-[7rem] font-black uppercase tracking-tighter leading-none italic text-purple-400 drop-shadow-[4px_4px_0px_black]"
      >
        Why <span class="text-white">Choose</span> Us?
      </h2>
      <p
        class="text-2xl font-black uppercase bg-purple-600 text-white inline-block px-6 py-2 rotate-1 shadow-[4px_4px_0px_0px_black]"
      >
    ...
    >
      ...
      <BentoCard colorClass="bg-orange-500" class="md:col-span-2">
        <div
          class="grow flex items-center justify-center p-8 bg-orange-400/20 relative overflow-hidden"
        >
          <UIcon
            name="i-lucide-star"
            class="absolute -right-6 -top-6 text-purple-300 opacity-40 group-hover:scale-125 transition-transform duration-700 w-36 h-36"
          />
          <div
            class="bg-white border-4 border-black p-5 flex items-center gap-5 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] rotate-2 group-hover:rotate-0 transition-transform relative z-10 w-full"
          >
            <div class="bg-yellow-400 border-4 border-black p-3 flex shrink-0">
              <UIcon name="i-lucide-trophy" class="w-8 h-8" />
            </div>
            <div class="flex flex-col overflow-hidden">
              <span class="text-xs font-black uppercase text-orange-600"
                >Achieved!</span
              >
        ...
        <div class="bg-orange-700 p-8 border-t-4 border-black text-white">
          <h3
            class="text-3xl font-black uppercase drop-shadow-[2px_2px_0_black]"
          >
            AOTC Raiding
          </h3>
          <p class="text-sm font-bold opacity-90">
            2 Nights a week, AotC every tier.
          </p>
        </div>
      </BentoCard>

      <!-- M+ Card -->
      <BentoCard colorClass="bg-blue-500" class="md:col-span-2">
        <div
          class="grow p-8 space-y-4 flex flex-col justify-center bg-blue-400/30"
        >
          <div
            class="bg-white border-4 border-black p-3 flex items-center gap-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] group-hover:translate-x-2 transition-transform w-[90%]"
          >
            <span
        ...
        <div class="bg-blue-700 p-8 border-t-4 border-black text-white">
          ...
          <p class="text-sm font-bold opacity-90">
            Multiple groups for running keys.
          </p>
        </div>
      </BentoCard>

      <!-- Stream Card -->
      <BentoCard colorClass="bg-pink-500" class="md:col-span-2">
        <div class="grow flex items-center justify-center p-8 bg-pink-400/20">
          <div
            class="bg-white border-4 border-black p-6 w-full shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] group-hover:rotate-1 transition-transform"
          >
            <div class="flex justify-between items-center mb-4">
              <span class="text-[10px] font-black uppercase text-pink-600"
                >Weekly Stream</span
              >
              <div
                class="bg-red-600 px-2 py-0.5 border-2 border-black animate-pulse flex items-center gap-1"
              >
            ...
            <h4
              class="text-xl font-black uppercase leading-none mb-3 text-black"
            >
              Raid Night Recap
            </h4>
            <div
              class="bg-pink-600 text-white font-black text-center py-2 text-xs border-2 border-black"
            >
              WED/SUN 8PM EST
            </div>
          </div>
      ...
      >
        <div class="grow flex items-center justify-center p-8 bg-purple-400/20">
          <div class="flex -space-x-6">
            <div
              v-for="(cat, i) in ['🐱', '🦁', '🐈']"
              :key="i"
              class="w-16 h-16 rounded-full border-4 border-black bg-white flex items-center justify-center text-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group-hover:-translate-y-2 transition-transform"
            >
              {{ cat }}
            </div>
          </div>
        ...
        <div class="bg-purple-700 p-8 border-t-4 border-black text-white">
          ...
          <p class="text-sm font-bold opacity-90">
            Unwritten guild rule, must love cats.
          </p>
        </div>
      </BentoCard>
    </div>

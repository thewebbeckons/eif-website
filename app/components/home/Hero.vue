...
const generateWipeWisdom = async () => {
  ...
  try {
    ...
    wipeMotivation.value = "Even the void is disappointed in that pull. Try pressing buttons next time?"
  } catch (e) {
...
<template>
  <section class="relative flex flex-col lg:flex-row items-center justify-between gap-12 mt-8 md:mt-12 min-h-[60vh] pointer-events-none">
    <div class="max-w-3xl space-y-8 z-10 relative pointer-events-auto">
      <h2 class="text-5xl sm:text-6xl md:text-7xl lg:text-[6.5rem] font-black uppercase leading-[0.85] tracking-tight text-white drop-shadow-[4px_4px_0px_#a855f7]">
        ...
        <span class="text-purple-400 relative">
          ...
          <span style="-webkit-text-stroke: 2px black" class="drop-shadow-[6px_6px_0px_rgba(0,0,0,1)]">US-Illidan</span>
        </span>
      </h2>
    ...
    <div class="relative w-full lg:w-1/2 flex flex-col items-center justify-center gap-12 z-10 pointer-events-auto">
      <!-- Loot Card -->
      <LootCard />
